...
plug_map.normal = {
    ...
    ["g"] = {
        ...
        t = bind.convert_wk_format(
            map_cr("lua require('telescope.builtin').lsp_type_definitions()")
                :with_noremap()
                :with_silent()
    ...
    ["<leader>d"] = {
        ...
        d = bind.convert_wk_format(
            map_cr("lua require('telescope.builtin').diagnostics({bufnr=0})")
                :with_noremap()
                :with_silent()
                :with_label("Show Diagnostics In Current Buffer")
        ),
        e = bind.convert_wk_format(
            map_cr("lua require('telescope.builtin').diagnostics({ignore_filename=true})")
                :with_noremap()
                :with_silent()
                :with_label("Show Diagnostics In WorkSpace")
        ),
        t = bind.convert_wk_format(
            map_cr("lua require('lspsaga.diagnostic').toggle_virtual_text()")
                :with_noremap()
                :with_silent()
...
plug_map.visual = {
    ["g"] = {
        name = "Lsp Function",
        f = bind.convert_wk_format(
            map_cr("lua vim.lsp.buf.formatting()")
                :with_noremap()
                :with_silent()
