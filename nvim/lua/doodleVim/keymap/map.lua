...
map.Lsp = {
  ...
  v = {
    ["g"] = {
      ...
      a = bind.convert_wk_format(
        map_cu("lua vim.lsp.buf.range_code_action()"):with_noremap():with_silent():with_label("Range Code Action")
      ),
      n = bind.convert_wk_format(
        map_cr("lua require('rename').rename()"):with_noremap():with_silent():with_label("Rename")
      ),
...
map.Default = {
  ...
  i = {
    ...
    ["<C-s>"] = bind.convert_wk_format(
      map_cmd('<Esc>:lua require("doodleVim.extend.misc").safe_save()<CR>'):with_noremap()
    ),
    ["<C-q>"] = bind.convert_wk_format(
      map_cmd('<Esc>:lua require("doodleVim.extend.misc").safe_exit()<CR>'):with_noremap()
    ),
  },
  c = {
    ["<C-b>"] = bind.convert_wk_format(map_cmd("<Left>"):with_noremap()),
    ...
    ["<C-t>"] = bind.convert_wk_format(
      map_cmd([[<C-R>=expand("%:p:h") . "/" <CR>]]):with_noremap():with_label("Show Current Directory")
    ),
  },
  v = {
    ["<C-d>"] = bind.convert_wk_format(
      map_cmd("<cmd>lua ensure_require('neoscroll').scroll(vim.wo.scroll, true, 150)<CR>")
        :with_noremap()
        :with_silent()
        :with_label("Smooth Scroll Down")
    ),
