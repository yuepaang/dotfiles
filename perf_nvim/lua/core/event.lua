...
function autocmd.load_autocmds()
    local definitions = {
        packer = {{"BufWritePost", "*.lua", "lua require('core.pack').auto_compile()"}},
        ...
        ft = {{"BufNewFile,BufRead", "*.toml", " setf toml"}, {"BufReadPost,BufNewFile", "*.sol", " setf solidity"},
              {"FileType", "Outline", " setlocal signcolumn=no"}, {"FileType", "python", " setlocal colorcolumn=80"}},

        yank = {{"TextYankPost", [[* silent! lua vim.highlight.on_yank({higroup="IncSearch", timeout=400})]]}},
        action = {{"CursorHold,CursorHoldI", "*", "lua require'nvim-lightbulb'.update_lightbulb()"}},

        nonewline = { -- don't auto comment new line
        {"BufEnter", [[set formatoptions-=cro]]}},

        oldlocations = {{"BufReadPost", "*",
